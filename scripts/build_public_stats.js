...
const norm=x=>{const d=String(x||'').replace(/\D/g,'');if(d.length===10)return '+52'+d;if(d.length===12&&d.startsWith('52')) return '+'+d; return null;};
...
const community=pool.size,officialCount=official.size,mon=officialCount+community;const display=mon>=10000?'10000+':(mon>=3000?'3000+':String(mon));
