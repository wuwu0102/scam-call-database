...
const HEADER='number,label,sourceName,sourceUrl,region,note,confidence,category';const target=10000;const cats=['spam','telemarketing','cobranza','publicidad','robocall','scam','fraud','extortion','unknown'];
...
if(pool.size<target){
  ...
  for(const r of pending){
    const n=norm(r.number);if(!n||official.has(n)||bulkSet.has(n)) continue;
    extra.push([n,'suspicious',r.sourceName||'Reporte comunitario',r.sourceUrl||'',r.region||'México',r.note||'Reporte comunitario en revisión',String(r.confidence||0.35),cats.includes(String(r.category).toLowerCase())?String(r.category).toLowerCase():'unknown']);
  }
  for(const r of extra){bulkSet.add(r[0]);bulkRows.push(r);added++;add(r[0],r[7]);if(pool.size>=target)break;}
}
fs.writeFileSync(BULK,HEADER+'\n'+bulkRows.map(toLine).join('\n')+'\n');
const report={generatedAt:new Date().toISOString(),target,communitySignalPoolCount:pool.size,remainingGap:Math.max(0,target-pool.size),addedToBulkFromSnapshots:added,categoryBreakdown:catBreak,officialCount:official.size,pendingCount:pending.length,communityBulkCount:bulkSet.size,snapshotCount:new Set(snapRows.map(r=>norm(r[0])).filter(Boolean)).size,skippedDuplicates:dup,skippedOfficialExisting:skipOff,skippedInvalid:skipInvalid,sourcesUsed:[...new Set(snapRows.map(r=>r[2]).filter(Boolean))]};
